...
## Post Recipe
Creates new Recipe with the first post of the author and returns the recipe as json.
...
```postText``` : [string] -- The text of your post. This post will be append as first comment on your Recipe.
...
``` 
{
    "saves": ["5f86c1f0a112c130e89964af"],
...
## Create Post
Creates new Post of the author and returns the recipe as json.
...
.post /recipes - create new recipe only for registered users

posts:
.get recipes/id - get all posts for recipe
.post recipes/id - create post in recipe by id only for registered users
.put recipes/id/posts/id - edit post only possible for author
.delete recipes/id/posts/id - delete post only possible for author -->
...
<!--http://localhost:3000/api/recipes -- {"recipeName":"Some Recipe", "userId":"5f85bf709a517d36f4abe656", "post": "Some Post" } -->
<!-- http://localhost:3000/api/recipes/5f858dd2d895ad23602db9d4  -- {"userId":"5f8580d25d1da62568dd38fd", "postText": "Some Post textsdfasdf" } -->
